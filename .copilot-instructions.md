...
- The coding assistant cannot execute **Docker commands** against the NAS/host environment.
- Non-Docker host commands (e.g., `bash`, `vault`, `jq`, `openssl`, file checks) may still be suggested for manual execution.
- Do not assume command execution happened unless the user confirms output.
- For Docker-related validation, troubleshooting, or deployment steps, provide **copy/paste Bash commands** for the user to run manually.
- Prefer Bash examples over PowerShell unless the user explicitly asks for PowerShell.
- When suggesting commands, group them as:
...
- [doc/ARCHITECTURE.md](doc/ARCHITECTURE.md) - Detailed platform architecture
- [doc/README.md](doc/README.md) - Setup and usage guide
- [doc/keycloak/KEYCLOAK_SETUP.md](doc/keycloak/KEYCLOAK_SETUP.md) - Keycloak configuration
- [doc/vault/PKI_SETUP_VAULT.md](doc/vault/PKI_SETUP_VAULT.md) - PKI engine setup
- [config/vault/vault-config.hcl](config/vault/vault-config.hcl) - Vault configuration
- [doc/vault/README_VAULT_PKI.md](doc/vault/README_VAULT_PKI.md) - Certificate generation guide
